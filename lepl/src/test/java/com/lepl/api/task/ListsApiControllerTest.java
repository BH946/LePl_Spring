...
//        t1.setId(1L);
    ...
    when(t1.getContent()).thenReturn("test");
    when(t1.getStartTime()).thenReturn(localDateTime);
    when(t1.getEndTime()).thenReturn(localDateTime);
    when(t1.getTaskStatus()).thenReturn(taskStatus);
    l1.addTask(t1);
    listsList.add(l1);
  }
  ...
  public void 회원의_해당날짜범위의_Lists_조회() throws Exception {
    ...
    mockMvc.perform(
            post("/api/v1/lists/member/date")
                .session(s)
                .contentType(MediaType.APPLICATION_JSON)
                .accept(MediaType.APPLICATION_JSON) //json 형태 사용
                .characterEncoding("UTF-8")
                .content(content) //json 데이터
        )
        .andExpect(status().isOk())
        .andDo(print());

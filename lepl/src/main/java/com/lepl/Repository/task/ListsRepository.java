...
public class ListsRepository {
  ...
  /**
   * 이 findByCurrent 함수가 Lazy+컬렉션이고, fetch join 을 안해서 좀 더 효과적인 경우 => 물론 fetch join 써도 되지만, 여기선 lists
   * 존재 여부만 확인하면 되므로 애초에 추가로 쿼리 날라갈게 없이 1개로 끝남. => 이것이 굳이 전부 Lazy 로 만들었을때의 장점 중에 하나
   * <p>
  ...
  public List<Lists> findByDateWithMemberTask(Long memberId, LocalDateTime start,
      LocalDateTime end) {
    return em.createQuery(
            "select distinct l from Lists l" +
                " join fetch l.tasks t" +

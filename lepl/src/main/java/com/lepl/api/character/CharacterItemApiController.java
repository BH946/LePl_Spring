...
public class CharacterItemApiController {
  ...
  public ResponseEntity<String> buyItem(@PathVariable("itemId") Long itemId, @Login Long memberId) {
    ...
    for (CharacterItem i : items) {
      if (i.getItemId() == wantItemId
          && i.getCharacter().getId() == character.getId()) { //만약 이미 해당 아이템을 소유하고 있다면
  ...
  public ResponseEntity<String> putItem(@PathVariable("itemId") Long itemId,
      @PathVariable("status") int status, @Login Long memberId) {
    Member member = memberService.findOne(memberId);
    ...
    for (CharacterItem c : characterItems) {
      if (c.getItemId() == item.getId()) { //캐릭터가 아이템을 보유하고 있다면
        characterItem = c;
        break; //어차피 아이템은 사용자 한 명 당 1개만 소유할 수 있기에 찾으면 바로 탈출
      }
    }

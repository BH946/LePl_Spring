...
import static com.lepl.util.Messages.FAIL_FOLLOW;
import static com.lepl.util.Messages.SUCCESS_FOLLOW;
import static com.lepl.util.Messages.SUCCESS_FOLLOW_CANCEL;
import static com.lepl.util.Messages.UNAUTHORIZED_FOLLOW_CANCEL;
...
public class FollowApiController {
  ...
  /**
   * 팔로우 생성 API -> 알림 포함! 입력 -> JSON followingId를 입력 받는다.
   */
  @PostMapping("/new")
  public ResponseEntity<String> create(@Login Long memberId,
    ...
    Notification notification = Notification.createNotification(findCharacter,
        "테스트 알림 : " + member.getNickname() + "가 팔로우 하였습니다.");
    notificationService.join(notification);
    return ResponseEntity.status(HttpStatus.CREATED).body(SUCCESS_FOLLOW);
  }

  /**
   * 팔로우 제거 API 입력 -> JSON followId를 입력 받는다. 참고) 꼭 자신의 캐릭터 ID 를 FK 로 가지는 Follow 테이블만 삭제 가능하게 검증 필수
   */
  @PostMapping("/delete")

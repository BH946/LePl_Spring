...
public class ExpApiController {
  ...
  /**
   * 경험치 업데이트 API - 일반 일정 완료 (List 형태이므로 여러개 업데이트 가능) 입력 -> JSON taskId 필수
   */
  @PostMapping("/tasks")
  public ResponseEntity<ResExpDto> successTasks(@Login Long memberId,
      @RequestBody List<ReqTaskDto> request) {
    ...
    for (Task task : tasks) {
      if (task.getTaskStatus().getCompletedStatus()) {
        continue; // 이미 이전에 완료했던 일정이라 pass
      }
  ...
  /**
   * 경험치 업데이트 API - 일정 타이머 종료 (타이머 특성상 1개씩 완료) 입력 -> JSON taskId, useTime 필수 -> 밀리세컨 단위
   */
  @PostMapping("/timer")
  public ResponseEntity<ResExpDto> successTimer(@Login Long memberId,
      @RequestBody ReqTimerDto request) {
    Member member = memberService.findOne(memberId); //캐릭터 화폐 업데이트를 위함
    Exp exp = expService.findOneWithMember(memberId); // 영속 exp
    ...
    Long timerAllUseTime =
        task.getLists().getTimerAllUseTime() + request.getUseTime(); // 타이머총사용시간 단위 : 밀리세컨 단위
    ...
    calTimeDto.calTime();
    // Exp 업데이트
    taskService.updateStatus(task, calTimeDto.getCompleted(), calTimeDto.getTimerOnOff(),
        calTimeDto.getRemainTime()); // 더티체킹
    listsService.updateTime(task.getLists(), calTimeDto.getTimerAllUseTime(),
        calTimeDto.getCurTime()); // 더티체킹
    exp = expService.update(exp, calTimeDto.getPointTask(), calTimeDto.getPointTimer()); // 더티체킹
    characterService.updateCoin(money + calTimeDto.getPointTask() + calTimeDto.getPointTimer(),
        character.getId()); //더티 체킹
